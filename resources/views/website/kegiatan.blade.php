                ...
                <div class="col-lg-6 aos-init aos-animate" data-aos="fade-up" data-aos-delay="400">
                    <div class="team-member d-flex align-items-start">
                        <div class="pic">
                            ...
                            @else
                            <img src="{{ Storage::url($item->thumbnail) }}" class="img-fluid" alt="{{ $item->judul }}">    
                            @endif
                        </div>
                        <div class="member-info">
                            <h4>
                                <a href="{{ url('kegiatan/' . $item->id) }}">{{ $item->judul }}</a>
                            </h4>
                            <span>
                                <i class="bi bi-calendar me-1 align-items-start"></i>
                                {{ \Carbon\Carbon::parse($item->tanggal_kegiatan)->format('d M Y') }}
                            </span>
                            <p>{{ \Illuminate\Support\Str::limit($item->deskripsi, 150, '...') }}</p>
                            <br>
                            <a href="{{ url('kegiatan/' . $item->id) }}" class="btn btn-sm btn-secondary">
